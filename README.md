...
- conv_helper.py，包含：
  - 卷积层conv_layer的定义
  - 残差层residual_layer的定义 
  - 反卷积层deconvolution_layer的定义
- model.py，包含：
  - 生成器generator的模型结构
...
2. 测试
   ...
   将可用的模型放到Checkpoints文件夹中（假设训练的模型迭代次数为**1000**，则模型含三个文件：**-1000.data-00000-of-00001**，**-1000.index**，**-1000.meta**）

   必要时修改文件夹中checkpoint文件：

   ```model_checkpoint_path: "-1000"```

   假设想将模型更换为**2000**次迭代的模型，则将checkpoint修改成：

   ```model_checkpoint_path: "-2000"```

   并确保Checkpoints文件夹中含有：
   - **-2000.data-00000-of-00001**
   -   **-2000.index**
...
- conv_helper.py，includes：
  ...
  - definition of residual layer
  - definition of deconvolution layer
